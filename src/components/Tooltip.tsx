...
const definitions: Record<string, { zh: string; en: string }> = {
  ...
  'open-subset-image': {
    zh: '想象一个球面（如地球仪），把南北两极挖掉，剩下的部分就是S²的一个"开子集"。然后用公式ψ把这个"去极球面"变形、嵌入到六维空间E⁶中，得到的曲面就是它的"像"。为什么要挖掉极点？因为公式中的tan在极点处趋于无穷，那里没有定义。',
    en: 'Imagine a sphere (like a globe)—remove the north and south poles, and the remaining part is an "open subset" of S². Then the formula ψ deforms and embeds this "punctured sphere" into 6D space E⁶; the resulting surface is its "image". Why remove the poles? Because tan in the formula diverges there, so it\'s undefined at those points.'
  },
};
